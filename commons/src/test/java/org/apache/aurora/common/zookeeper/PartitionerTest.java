...
public class PartitionerTest extends BaseZooKeeperTest {
  ...
  private static class InstrumentedPartitioner extends Partitioner {
    ...
    @Override
    Group.GroupChangeListener createGroupChangeListener(Group.Membership membership) {
      final Group.GroupChangeListener listener = super.createGroupChangeListener(membership);
      return new Group.GroupChangeListener() {
  ...
  public void testJoin() throws Exception {
    ...
    Partitioner.Partition firstPartition = join(firstPartitioner);
    ...
    Partitioner.Partition secondPartition = join(secondPartitioner);
    ...
    Partitioner.Partition thirdPartition = join(thirdPartitioner);
  ...
  private Partitioner.Partition join(Partitioner partitioner) throws Group.JoinException, InterruptedException {
    final Partitioner.Partition partition = partitioner.join();
    addTearDown(new TearDown() {
      @Override public void tearDown() throws Group.JoinException {
        partition.cancel();
      }
    });
    return partition;
