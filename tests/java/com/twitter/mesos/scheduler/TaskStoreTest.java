...
public class TaskStoreTest {
  ...
  public void testAddAndFetchTasks() {
    ...
    Predicate<ScheduledTask> taskIdFilter = new Predicate<ScheduledTask>() {
      @Override public boolean apply(ScheduledTask task) {
        return task.getAssignedTask().getTaskId() == 1;
      }
    };
  ...
  public void testMutate() {
    ...
    // Mutate by query.
    taskStore.mutate(getById(TASK_A_ID), new Closure<ScheduledTask>() {
    ...
    // Mutate by reference.
    ScheduledTask immutable = Iterables.getOnlyElement(taskStore.fetch(getById(TASK_B_ID)));
    taskStore.mutate(immutable, new Closure<ScheduledTask>() {
