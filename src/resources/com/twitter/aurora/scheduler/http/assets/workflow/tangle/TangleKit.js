...
//  TKSelect
//
//  Allow the user to select from a set of values.
//
Tangle.classes.TKSelect = {
  initialize: function (element, options, tangle, variable) {
    this.input = new Element('select', {});
    if(element.hasChildNodes()) {
      this.input.innerHTML = element.innerHTML;
      element.innerHTML = "";
    }
    this.input.inject(element, "top");

    var inputChanged = (function () {
      var e = this.input;
      tangle.setValue(variable, e.options[e.selectedIndex].value);
    }).bind(this);
...
//  or Yes or no if option.type is text.
    ...
    initialize: function (element, options, tangle, variable) {
      element.addEvent("click", function (event) {
          ...
          if (options.type="text") {
            tangle.setValue(variable, (isActive == "Yes" || isActive == 1) ? "No" : "Yes");
          } else {
...
//  TKTextField
//
//  An input box where text can be typed in.
//
//  Attributes:  data-size (optional): width of the box in characters

  Tangle.classes.TKTextField = {

    initialize: function (element, options, tangle, variable) {
      this.input = new Element("input", {
        type: options.type || "text",
        "class":"TKTextFieldInput",
...
//  TKTextAreaField
//
//  An input box where multiline text can be entered.
//
//  Attributes:  data-size (optional): width of the box in characters

  Tangle.classes.TKTextArea = {

    initialize: function (element, options, tangle, variable) {
      this.input = new Element("textArea", {
        "class":"TKTextAreaInput",
        rows: options.rows,
        cols: options.cols
      }).inject(element, "top");
      var inputChanged = (function () {
