...
class AuroraCommandContext(Context):
  ...
  def _get_config_with_production_and_tier(self, cluster, config):
    ...
    if task.tier is None:
      backfill_args = {
        'tier': String(next((t.name for t in tier_configurations.tiers if
          not to_bool(t.settings['preemptible']) == task.production and
          not to_bool(t.settings['revocable'])), tier_configurations.defaultTierName))
      }
    else:
      backfill_args = {
      'production': Boolean(
